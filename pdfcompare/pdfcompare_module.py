...
def pdf2xml(infile, key='', firstpage=None, lastpage=None):
  ...
  """
  dom = do_pdf2xml(infile, key=key, firstpage=firstpage, lastpage=lastpage, relaxed=False)
  if dom is None:
    print(" pdf2xml retrying more relaxed ...")
    dom = do_pdf2xml(infile, key=key, firstpage=firstpage, lastpage=lastpage, relaxed=True)
  return dom

def do_pdf2xml(infile, key='', firstpage=None, lastpage=None, relaxed=False):
  """ read a pdf file with pdftohtml and parse the resulting xml into a dom tree
      the first parameter, parser is only used for calling exit() with proper messages.
  ...
  try:
    pdf_xml = subprocess.check_output(pdftohtml_cmd + [infile])
  except Exception as e:
    print(" ".join(pdftohtml_cmd + [infile]))
    raise "pdftohtml -xml failed: " + " ".join(pdftohtml_cmd + [infile]) + ": " + str(e)

  # xmlByteString = b''.join([line.strip() for line in from_child.splitlines() if line.strip()])
  data = pdf_xml.decode(errors="ignore")
  try:
    if relaxed:
      data = re.sub("(<a.*?>|</a>)","", data)      # <a...> </a> appear to be misplaced.
    ...
    print(" ".join(pdftohtml_cmd + [infile]))
    if relaxed:
      raise "ElementTree parsing failed.\nET.parse: " + str(e) + ")\n\n"
    else:
      return None
  print("pdf2xml done")
  return dom
...
def compare_pdfs(old_pdf, new_pdf):
  return main(infile=old_pdf, infile2=new_pdf, compare_text=True, no_output=True)
...
def main(
  ...
  decrypt_key: str='',
  exclude_irrelevant_pages: bool=False,
  features: str='H,C,P,W,B',
  nocase: bool=False,
  log: str=None,
  mark: str='A,D,C',
  ...
  """Compare two PDF Files and return the result in an annotated new file

  Returns tuple[PdfFileWriter | Any, int]: 
    A tuple consisting of the resulting PDF in a PdfFileWriter object and the number of changes
    detected between the two documents

  Parameters:
    - infile (str): Path to the first input file.
    - infile2 (str): Path to optional 'Newer' input file for comparison.
    - compare_text (bool): Mark added, deleted and replaced text (or see -m) with regard to OLDFILE.
      File formats .pdf, .xml, .txt are recognized by their suffix.
      The comparison works word by word.\\
      Default: False
    - decrypt_key (str): Open an encrypted PDF by providing the key.\\
      Default: ''
    - exclude_irrelevant_pages: Show only changed pages. With -s: show only matching pages.\\
      Default: False - reproduce all pages from infile1 in output_file.
    - features (str): Specify how to mark. 
      Allowed values are 'highlight', 'changebar', 'popup', 'navigation', 'watermark', 'margin'.\\
      Default: 'H,C,P,W,B'
    - nocase (bool): Make 'search' option case insensitive.\\
      Default: False - case sensitive.
    - log (str): Path to logfile - write a python datastructure describing all the overlay objects on each page.\\
      Default: None
    - mark (str): Specify what to mark. Used when infile2 is provided for comparison.
      Allowed values are 'add','delete','change','equal'.
      Multiple values can be listed comma-seperated; abbreviations are allowed.\\
      Default: 'A,D,C'
    - no_output (bool): Do not write an output file; print diagnostics only.\\
      Default: write output file as per 'output_file' option.
    - search (str): Highligh matching words (regular expression)\\
      Default: None
    - spell_check (bool): Run the text body of the (new) pdf through hunspell. Unknown words are underlined.
      ...
      Default: False
    - strict (bool): Show really all differences. Default: ignore removed hyphenation;
      ignore character spacing inside a word.\\
      Default: False
    - transparency (float): Set transparency of the highlight; invisible: 0.0; full opaque: 1.0.\\
      Default: 0.6
    - below (bool): Paint the highlight markers below the text. Try this if the normal merge crashes. 
      Use with care, highlights may disappear below background graphics.\\
      Default: False
    - search_colors (object): Set colors of the search highlights as an RGB triplet;
      R,G,B ranges are 0.0-1.0 each; valid names are 'add,'delete','change','equal','margin','all';
      default name is 'equal', which is also used for the 'search' option.\\
      Default: { \\
        'E': [1,0,1,    'pink'],        # extra\\
        'A': [.3,1,.3,  'green'],       # added\\
        'D': [1,.3,.3,  'red'],         # deleted\\
    ...
    - _debug (bool): Enable debugging. Prints more on stdout, dumps several *.xml or *.pdf files.\\
      Default: False
    - first_page (int): Skip some pages at start of document.\\
      Default: None - All pages.
    - last_page (int): Limit pages processed; this counts pages, it does not use document page numbers.\\
      Default: None - All pages.
    - margins (str): Specify margin space to ignore on each page. A margin width is expressed
      in units of ca. 100dpi. Specify four numbers in the order north,east,west,south.\\
      Default: '0,0,0,0'
    - source_location (bool): Annotation start includes :pNX: markers where 'N' is the page number of the location
      in the original document and X is 't' for top, 'c' for center, or 'b' for bottom of the page.\\
      Default: False - Annotations start only with 'chg:', 'add:', 'del:' optionally followed by original text.
    - no_compression (bool): "Write uncompressed PDF.\\
      Default: False - FlateEncode filter compression.
    - leftside (bool): Put changebars and navigation at the left hand side of the page. \\
      Default: False - Right hand side.
  """

  transparency = 1 - transparency     # it is needed reversed.

  global debug 
  debug = _debug

  global anno_popup_src_loc_ref
  anno_popup_src_loc_ref = source_location

  margins = parse_margins(margins, search_colors['B'])
  ...
  if compare_text == True and infile2 is not None:
    compare_text,infile = infile,infile2

  if search is None and compare_text is None and spell_check is None:
    raise "Oops. Nothing to do. Specify either -s or --spell or -c or two input files."

  if not os.access(infile, os.R_OK):
    raise ("Cannot read input file: %s" % infile)
  dom1 = pdf2xml(infile, key=decrypt_key, firstpage=first_page, lastpage=last_page)
  dom2 = None
  wordlist2 = None
  if compare_text:
    if re.search('\.pdf$', compare_text, re.I):
      dom2 = pdf2xml(compare_text, key=decrypt_key, firstpage=first_page, lastpage=last_page)
    ...
    elif re.search('\.xml$', compare_text, re.I):
      wordlist2 = xmlfile2wordlist(compare_text)
    else:
      # assuming a plain text document
      wordlist2 = textfile2wordlist(compare_text)

  if debug:
    dom1.write(output_file + ".1.xml")
    if dom2:
      dom2.write(output_file + ".2.xml")
  ...
  if input1.getIsEncrypted():
    if input1.decrypt(decrypt_key):
      if len(decrypt_key):
        print("Decrypted using key='%s'." % decrypt_key)
    else:
      raise ("decrypt(key='%s') failed." % decrypt_key)

  # last_page,first_page start counting at 0,
  # args.last_page, args.first_page start counting at 1.
  last_page = input1.getNumPages()-1
  first_page = 0
  if last_page and int(last_page) < last_page:
  ...
  page_marks = pdfhtml_xml_find(dom1, re_pattern=search, 
      wordlist=wordlist2,
      nocase=nocase,
      first_page=first_page,
      last_page=last_page,
  ...
  for i in range(first_page,last_page+1):
    if exclude_irrelevant_pages and len(page_marks[i]['rect']) == 0:
      continue
    ...
    page_watermark(c, cbox, sys.argv, color=search_colors['E'], trans=transparency, 
                   p_w=page_marks[i]['w'], p_h=page_marks[i]['h'], margins=margins, features=features)
    page_changemarks(c, mbox, cbox, page_marks[i], i-first_page, trans=transparency, leftside=leftside, features=features)
    ...
    pdf_str.seek(0,0)
    if debug:
      open("canvas_%d.pdf"%i, 'w').write(pdf_str.getvalue())
      pdf_str.seek(0,0)
  ...
  if no_output is False:
    ...
    try:
      ...
      sys.exit(1)
    outputStream.close()
    print("%s (%s pages) written." % (output_file, pages_written))
...
class Hunspell():
    ...
    def _readline(self):
        ...
        if self.proc is None:
            raise SystemError("Hunspell_readline before _start")
        while True:
